...
from evaluation.artifacts.model_bundle import save_deeplog_bundle
...
def main() -> None:
    ...
    parser.add_argument("--epochs", type=int, help="Override epochs for a validation-only tuning run.")
    ...
    settings = config["detectors"]["deeplog"]; limit = args.train_normal_limit or int(settings["train_normal_limit"]); epochs = args.epochs or int(settings["epochs"]); train_normal = [trace for trace in train if not trace.ground_truth][:limit]
    ...
    model = LogOnlyDeepLog(DeepLogConfig(sequence_length=int(settings["sequence_length"]), embedding_dim=int(settings["embedding_dim"]), lstm_units=int(settings["lstm_units"]), epochs=epochs, batch_size=int(settings["batch_size"]), random_seed=int(config["random_seed"]))).fit([trace.sequence for trace in train_normal])
    ...
    bundle_manifest = save_deeplog_bundle(model, models_output / "deeplog")
    with (output / "predictions.csv").open("w",newline="",encoding="utf-8") as handle:
        writer=csv.DictWriter(handle,fieldnames=list(rows[0])); writer.writeheader(); writer.writerows(rows)
    metrics={"evaluation_phase":"development_validation_only","metric_split":"validation","hdfs_log_only_deeplog":_metrics(rows),"selected_validation_threshold":threshold,"train_normal_samples":len(train_normal),"deeplog_train_normal_limit":limit,"epochs":epochs,"validation_samples":len(validation)}
