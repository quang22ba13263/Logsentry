...
def main() -> None:
    ...
    detector = LogOnlyIsolationForest(feature_names, IsolationForestConfig(n_estimators=int(detector_config["n_estimators"]), max_samples=detector_config["max_samples"], contamination=detector_config["contamination"], random_seed=int(config["random_seed"]))).fit([trace.features for trace in train_normal])
    ...
    metrics = {"evaluation_phase": "development_validation_only", "metric_split": "validation", "hdfs_log_only_isolation_forest": _metrics(rows), "selected_validation_threshold": threshold, "train_normal_samples": len(train_normal), "validation_samples": len(validation)}
